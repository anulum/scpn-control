...
print(f"  Coupling K:  3.0 (supercritical, mean-field)")
print(f"  Frequency:   N(0, 1) rad/s")
print()
...
print(f"  K_base:     0.45 (exponential decay α=0.3)")
print(f"  ζ (driver): 0.5 (global field coupling)")
...
omega_layers = [
    OMEGA_N_16[m] + rng.normal(0, 0.1, N_per) for m in range(16)
]
...
for step in range(200):
    result = upde.step(
        theta_layers, omega_layers,
        psi_driver=0.0, pac_gamma=0.0,
    )
    ...
    if step % 50 == 49:
        ...
        print(
            f"    Step {step+1:4d}: R_mean={R_mean:.3f}, "
            f"V_global={V_global:.4f}"
        )
...
print(f"  lambda thresh:  0.0 (any growth triggers violation)")
print(f"  Max violations: 3 consecutive before refusal")
...
for i in range(8):
    ...
    print(f"    Tick {i+1}: approved={v.approved}, "
          f"lambda={v.lambda_exp:.4f}, violations={v.consecutive_violations}")
...
print(f"\n  {'Scenario':>16s}  {'β_N':>4s}  {'q95':>4s}  {'Risk':>5s}  "
      f"{'K_mean':>7s}  {'ΔK_max':>7s}")
...
for name, beta_n, q95, risk, mirnov in scenarios:
    ...
    print(f"  {name:>16s}  {beta_n:4.1f}  {q95:4.1f}  {risk:5.2f}  "
          f"{K_mean:7.4f}  {K_delta:7.4f}")
...
print()
print(f"  {'Tick':>5s}  {'R_global':>8s}  {'V_global':>8s}  {'lam_exp':>8s}  "
      f"{'b_N':>4s}  {'Guard':>6s}")
print(f"  {'---':>5s}  {'--------':>8s}  {'--------':>8s}  {'--------':>8s}  "
      f"{'----':>4s}  {'------':>6s}")
