...
def validate_ntm_reference(
    ...
    *,
    ...
    """Validate NTM evidence against persisted measured or public reference artifacts."""
...
def _validate_artifact(path: Path, payload: object, errors: list[dict[str, object]]) -> dict[str, object] | None:
    ...
    if payload.get("source") == "measured_ntm_campaign" and not _has_measured_campaign(payload):
        errors.append(
            {"path": str(path), "field": "campaign", "error": "measured NTM campaigns require machine and shot_id or campaign_id"}
        )
    ...
    if not _strictly_increasing_unit_grid(payload.get("rho_grid")):
        errors.append({"path": str(path), "field": "rho_grid", "error": "rho grid must be finite and strictly increasing on [0, 1]"})
    if not _valid_q_profile(payload.get("q_profile"), payload.get("rho_grid")):
        errors.append({"path": str(path), "field": "q_profile", "error": "q profile must be finite, positive, and length-matched to rho_grid"})
...
def canonical_artifact_sha256(payload: dict[str, object]) -> str:
    """Return the tamper-evident digest for an NTM reference artifact."""
...
def _valid_rational_surface(value: object) -> bool:
    ...
    if m <= 0 or n <= 0:
        return False
    if not all(_is_positive_finite(item) for item in (r_s, q, a, r0)) or not _is_finite(shear):
        return False
    if not _is_positive_finite(rho) or float(rho) >= 1.0:
        return False
...
def _valid_eccd_alignment(value: object) -> bool:
    ...
    return _is_nonnegative_finite(power) and _is_nonnegative_finite(current) and _is_positive_finite(width) and _is_nonnegative_finite(error)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Directory or JSON artifact containing persisted NTM reference evidence",
    )
    parser.add_argument(
        "--require-reference-artifacts",
        action="store_true",
        help="Fail if no NTM reference artifacts are present",
    )
    parser.add_argument("--output-json", help="Write JSON report to this path")
    parser.add_argument("--json-out", action="store_true", help="Emit JSON report")
    args = parser.parse_args(argv)

    report = validate_ntm_reference(args.artifact_root, require_reference_artifacts=args.require_reference_artifacts)
    if args.output_json:
