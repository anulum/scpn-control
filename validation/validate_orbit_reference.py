...
# SCPN Control — Orbit reference artifact validator
...
_ALLOWED_SOURCES = {"real_orbit_campaign", "documented_public_reference"}
...
_MAXIMUM_ERROR_METRICS = ("banana_width_relative_error", "first_orbit_loss_abs_error")
_MINIMUM_SCORE_METRICS = ("passing_trapped_classification_accuracy",)
_SHA256_RE = re.compile(r"^[0-9a-fA-F]{64}$")


def validate_orbit_reference(
    artifact_root: str | Path,
    *,
    require_reference_artifacts: bool = False,
) -> dict[str, Any]:
    """Validate orbit-following evidence against persisted references."""
    ...
    if require_reference_artifacts and not paths:
        errors.append({"path": str(root), "field": "artifact_root", "error": "no orbit reference artifacts found"})
...
def _validate_artifact(path: Path, payload: object, errors: list[dict[str, object]]) -> dict[str, object] | None:
    ...
    if source not in _ALLOWED_SOURCES:
        errors.append({"path": str(path), "field": "source", "error": "source must be real_orbit_campaign or documented_public_reference"})
    if source == "real_orbit_campaign" and not _has_campaign_reference(payload):
        errors.append({"path": str(path), "field": "campaign_artifact_uri", "error": "real orbit campaigns require campaign_artifact_uri"})
    ...
    if not _valid_units(payload.get("units")):
        errors.append({"path": str(path), "field": "units", "error": "units must declare orbit reference units"})
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--artifact-root",
        default=str(ROOT / "validation" / "reports" / "orbit_reference"),
        help="Directory or JSON artifact containing persisted orbit-following reference evidence",
    )
    parser.add_argument("--require-reference-artifacts", action="store_true", help="Fail if no orbit reference artifacts are present")
    parser.add_argument("--output-json", help="Write JSON report to this path")
    parser.add_argument("--json-out", action="store_true", help="Emit JSON report")
    args = parser.parse_args(argv)

    report = validate_orbit_reference(args.artifact_root, require_reference_artifacts=args.require_reference_artifacts)
    if args.output_json:
    ...
    if args.json_out:
        ...
        print(f"Orbit reference: {report['status']} reference_artifacts={report['reference_artifacts']}")
