        ...
        errors.append(
            {
                "path": str(path),
                "field": "source",
                "error": "source must be real_orbit_campaign or documented_public_reference",
            }
        )
    if source == "real_orbit_campaign":
        uri_error = reference_artifact_uri_error(payload.get("campaign_artifact_uri"), "campaign_artifact_uri")
        if uri_error is not None:
            errors.append({"path": str(path), "field": "campaign_artifact_uri", "error": uri_error})
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--require-reference-artifacts", action="store_true", help="Fail if no orbit reference artifacts are present"
    )
