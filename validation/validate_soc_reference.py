...
def validate_soc_reference(
    artifact_root: str | Path,
    *,
    require_reference_artifacts: bool = False,
) -> dict[str, Any]:
    """Validate SOC turbulence-learning evidence against persisted references."""
    ...
    if require_reference_artifacts and not paths:
        errors.append({"path": str(root), "field": "artifact_root", "error": "no SOC reference artifacts found"})
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = validate_soc_reference(args.artifact_root, require_reference_artifacts=args.require_reference_artifacts)
    if args.output_json:
    ...
    if args.json_out:
        ...
        print(f"SOC reference: {report['status']} reference_artifacts={report['reference_artifacts']}")
