...
# SCPN Control — Free-boundary reference artifact validator
...
def validate_free_boundary_reference(
    artifact_root: str | Path,
    *,
    require_reference_artifacts: bool = False,
) -> dict[str, Any]:
    """Validate free-boundary tracking artifacts against strict claim gates."""
    ...
    if require_reference_artifacts and not paths:
        errors.append(
            {"path": str(root), "field": "artifact_root", "error": "no free-boundary reference artifacts found"}
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--artifact-root",
        default=str(ROOT / "validation" / "reports" / "free_boundary_reference"),
        help="Directory or JSON artifact containing persisted free-boundary reference evidence",
    )
    parser.add_argument(
        "--require-reference-artifacts",
        action="store_true",
        help="Fail if no free-boundary reference artifacts are present",
    )
    parser.add_argument("--output-json", help="Write JSON report to this path")
    parser.add_argument("--json-out", action="store_true", help="Emit JSON report")
    args = parser.parse_args(argv)

    report = validate_free_boundary_reference(
    ...
    if args.json_out:
        ...
        print(f"Free-boundary reference: {report['status']} reference_artifacts={report['reference_artifacts']}")
