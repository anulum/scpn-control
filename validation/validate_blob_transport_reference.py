...
# SCPN Control — Blob transport reference artifact validator

"""Validate persisted scrape-off-layer blob-transport reference artifacts."""
...
def validate_blob_transport_reference(
    artifact_root: str | Path,
    *,
    require_reference_artifacts: bool = False,
) -> dict[str, Any]:
    """Validate SOL blob-transport evidence against persisted measured/reference artifacts."""
...
def canonical_artifact_sha256(payload: dict[str, object]) -> str:
    """Return the tamper-evident digest for a blob-transport reference artifact."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--artifact-root",
        default=str(ROOT / "validation" / "reports" / "blob_transport_reference"),
        help="Directory or JSON artifact containing persisted blob transport reference evidence",
    )
    parser.add_argument(
        "--require-reference-artifacts",
        action="store_true",
        help="Fail if no blob transport reference artifacts are present",
    )
    ...
    if args.json_out:
        ...
        print(f"Blob transport reference: {report['status']} reference_artifacts={report['reference_artifacts']}")
