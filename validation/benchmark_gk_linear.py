...
# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Benchmark Gk Linear
...
# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Linear GK Benchmark Suite
# © 1998–2026 Miroslav Šotek. All rights reserved.
# Contact: www.anulum.li | protoscience@anulum.li
# ORCID: https://orcid.org/0009-0009-3560-0851
...
def run_cyclone_base_case(n_ky: int = 12, n_theta: int = 32) -> dict[str, Any]:
    ...
    return {
        ...
        "dominant_mode": result.mode_type[int(np.argmax(result.gamma))] if result.gamma_max > 0 else "stable",
...
def run_multi_code_comparison() -> dict[str, Any]:
    ...
    return {
        ...
        "native_gk": {"gamma_max": float(gk_result.gamma_max), "chi_i": gk_fluxes.chi_i, "chi_e": gk_fluxes.chi_e},
        "quasilinear": {
...
def run_sparc_iter_scans() -> dict[str, Any]:
    ...
    for name, R0, a, B0, q, s_hat, T_keV in [
        ...
        species = [deuterium_ion(T_keV=T_keV, R_L_T=6.0, R_L_n=2.0), electron(T_keV=T_keV, R_L_T=6.0, R_L_n=2.0)]
        result = solve_linear_gk(
            species_list=species, R0=R0, a=a, B0=B0, q=q, s_hat=s_hat, n_ky_ion=6, n_theta=32, n_period=1
        )
