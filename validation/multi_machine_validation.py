# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Multi-Machine Validation Campaign
# ──────────────────────────────────────────────────────────────────────
...
class MultiMachineValidator:
    ...
    def _test_current_conservation(self, m: MachineConfig):
        ...
        self.results.append(
            ValidationResult(
                "current_conservation", m.name, "Rel Error", err, 0.01, passed, f"Total current conservation {err:.2%}"
            )
        )
    ...
    def _test_energy_conservation(self, m: MachineConfig):
        ...
        self.results.append(
            ValidationResult(
                "energy_conservation", m.name, "Rel Error", err, 0.01, passed, f"Power balance error {err:.2%}"
            )
        )
    ...
    def _test_diagnostic_reconstruction(self, m: MachineConfig):
        ...
        self.results.append(
            ValidationResult(
                "diagnostic_reconstruction", m.name, "Shape Error", err, 0.05, passed, f"EFIT LCFS error {err:.2%}"
            )
        )
