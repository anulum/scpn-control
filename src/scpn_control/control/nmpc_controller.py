# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Nonlinear Model Predictive Control
# ──────────────────────────────────────────────────────────────────────
...
class NonlinearMPC:
    def __init__(self, plant_model: Callable[[np.ndarray, np.ndarray], np.ndarray], config: NMPCConfig):
    ...
    def step(self, x: np.ndarray, x_ref: np.ndarray, u_prev: np.ndarray) -> np.ndarray:
        ...
        for k in range(1, self.N + 1):
            if np.any(self.x_traj[k] < self.config.x_min - 1e-3) or np.any(self.x_traj[k] > self.config.x_max + 1e-3):
                viol = True
                break
