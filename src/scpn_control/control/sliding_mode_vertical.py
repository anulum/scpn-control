# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Super-Twisting Sliding Mode Control (Vertical)
# ──────────────────────────────────────────────────────────────────────
...
class SuperTwistingSMC:
    ...
    Provides finite-time convergence to s = 0 and chattering-free continuous output.
    """
    ...
    def sliding_surface(self, e: float, de_dt: float) -> float:
        """Compute the sliding variable s = e + c * e_dot."""
        return e + self.c * de_dt

    def step(self, e: float, de_dt: float, dt: float) -> float:
        """
        Advance super-twisting algorithm.
        u = -alpha * |s|^0.5 * sign(s) + v
        v_dot = -beta * sign(s)
        """
        ...
        # Anti-windup on internal state v (helps if saturated for long periods)
...
class VerticalStabilizer:
    ...
    def step(self, Z_meas: float, Z_ref: float, dZ_dt_meas: float, dt: float) -> float:
        """
        Compute required coil correction voltage.
        Error is defined such that Z_meas -> Z_ref.
        e = Z_meas - Z_ref
        """
        e = Z_meas - Z_ref

        # We need to drive e to 0.
        # Plant is: m_eff d^2Z/dt^2 = -K Z + F_coil + d
        # K = n_index * mu0 * Ip^2 / (4 pi R0)
        # We assume the SMC provides the corrective voltage command directly
...
def lyapunov_certificate(alpha: float, beta: float, L_max: float) -> bool:
    """
    Verify gain conditions for finite-time convergence.
    alpha > sqrt(2 * L_max)
    beta > L_max
    """
