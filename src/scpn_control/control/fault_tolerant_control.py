# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Fault-Tolerant Reconfigurable Control
# ──────────────────────────────────────────────────────────────────────
...
class FDIMonitor:
    ...
    def __init__(self, n_sensors: int, n_actuators: int, threshold_sigma: float = 3.0, n_alert: int = 5):
        ...
        # Assume nominal S_i (innovation variance) is 1.0 for normalized inputs
    ...
    def update(self, y_measured: np.ndarray, y_predicted: np.ndarray, t: float) -> list[FaultReport]:
        """
        Check for sensor faults using prediction residual.
        """
        ...
        for i in range(self.n_sensors):
            ...
            # Check for sudden dropout or drift
            # A simple heuristic: if all recent samples are outside 3-sigma
            hist = self.innovation_history[:, i]
            sigma = np.sqrt(self.S_diag[i])

            if np.all(np.abs(hist) > self.threshold_sigma * sigma):
                # Is it a dropout (NaN or zero)?
...
class ReconfigurableController:
    """
    Adjusts control allocation and estimator based on detected faults.
    """
    ...
    def _compute_gain(self) -> np.ndarray:
        ...
        # Zero out rows for faulted coils so they don't get commands
    ...
    def step(self, error: np.ndarray, dt: float) -> np.ndarray:
        ...
        """
        # Adjust error for stuck coils
        adjusted_error = error.copy()
        for c_idx, val in self.stuck_values.items():
            # The stuck coil produces a fixed offset in the output: y_stuck = J[:, c_idx] * val
            # Target needs to be adjusted to compensate
            adjusted_error -= self.nominal_jacobian[:, c_idx] * val

        delta_u = self.K @ adjusted_error

        # Ensure zero command for faulted coils
    ...
    def controllability_check(self) -> bool:
        """Verify remaining actuators can span the necessary target space."""
        # Rank of reduced Jacobian should be at least the number of critical targets
        # For a simplified check, if we lose too many coils, it's uncontrollable
        ...
        # Require enough rank to control essential parameters
        min_required_rank = 2
        return bool(rank >= min_required_rank)

    def graceful_shutdown(self) -> np.ndarray:
        """Return ramp-down command."""
...
class FaultInjector:
    ...
    def inject(self, t: float, signals: np.ndarray) -> np.ndarray:
        ...
        if self.fault_type == FaultType.SENSOR_DROPOUT:
            ...
            # Drift grows linearly after fault time
