...
def kadomtsev_crash(
    ...
    # Find rho_mix where psi_star returns to 0 (for rho > rho_1)
    ...
    # Volume average T and n inside rho_mix
    # dV ~ rho drho
    ...
    T_new[:idx_mix] = T_mix
    n_new[:idx_mix] = n_mix
    q_new[:idx_mix] = 1.01  # Reset to slightly above 1

    # Smooth transition
    if idx_mix < len(rho) - 1:
        # Avoid sharp discontinuity at mixing radius
        pass
...
class SawtoothCycler:
    ...
    def step(self, dt: float, q: np.ndarray, shear: np.ndarray, T: np.ndarray, n: np.ndarray) -> SawtoothEvent | None:
        ...
        if self.monitor.check_trigger(q, shear):
            ...
            # 1e19 m-3 * 1e3 eV = 1e22 eV/m3 = 1600 J/m3
            ...
            # Energy should be conserved in Kadomtsev, so W_after ~ W_before
            # The "seed energy" is the kinetic energy released from the core
            # Let's define seed energy as the change in core energy (which moved outward)
            # Seed energy ~ Delta W_core
            seed_energy = max(W_before - W_after, 0.0)  # If perfectly conserved, it's 0.
            # Realistically, seed energy for NTMs is related to the core pressure drop
            core_energy_drop = (
