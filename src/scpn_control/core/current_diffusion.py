# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Current Diffusion Equation
# ──────────────────────────────────────────────────────────────────────
...
def neoclassical_resistivity(
    ...
    """
    Compute neoclassical parallel resistivity.
    Sauter, Angioni & Lin-Liu (1999/2002).
    """
    ...
    # Electron collisionality nu*_e
    # nu_ei = 0.51e-4 * ne * Z_eff * ln_Lambda / Te_eV^1.5
    e_charge = 1.602e-19
    m_e = 9.109e-31
    ...
    nu_star_e = nu_ei * max(q, 0.5) * R0 / (epsilon**1.5 * v_te)

    # Neoclassical correction C_R
    C_R = 1.0 - (1.0 + 0.36 / Z_eff) * f_t + (0.59 / Z_eff) * f_t**2
    # Adjust C_R for collisionality? Sauter 1999 includes collisionality in C_R
    # but the simplest Sauter 2002 analytical limit is the one given in prompt:
    # C_R = 1 - (1 + 0.36/Z_eff) f_t + 0.59/Z_eff * f_t^2
    # We will just use this explicit formula as requested.
...
def q_from_psi(rho: np.ndarray, psi: np.ndarray, R0: float, a: float, B0: float) -> np.ndarray:
    """
    Compute q(rho) = -rho * a^2 * B0 / (R0 * dpsi/drho)
    Uses L'Hopital's rule at the axis.
    """
    nr = len(rho)
    q = np.zeros(nr)
    drho = rho[1] - rho[0]
    ...
    for i in range(1, nr):
        ...
        if abs(denom) < 1e-12:
            ...
            # We assume the prompt's "-rho B0 / (R0 dpsi/drho)" missed an a^2 factor
            # for dimensional consistency if rho is normalized. We include a^2.
            q[i] = -rho[i] * a**2 * B0 / denom

    # L'Hopital at rho=0
    # q(0) = -a^2 B0 / (R0 * d^2psi/drho^2)
...
def resistive_diffusion_time(a: float, eta: float) -> float:
    """Characteristic resistive timescale tau_R = mu0 a^2 / eta."""
    return MU_0 * a**2 / max(eta, 1e-12)


class CurrentDiffusionSolver:
    """
    Implicit Crank-Nicolson solver for the 1D poloidal flux diffusion equation.
    """
    ...
    def __init__(self, rho: np.ndarray, R0: float, a: float, B0: float):
        ...
        # Initialize with parabolic current profile -> psi ~ (1 - rho^2)^2 type
        # Or just a generic psi profile
        self.psi = np.zeros(self.nr)
        # Set a default q=1 at axis, q=3 at edge profile
        # psi(rho) = - a^2 B0 / R0 * integral( rho / q(rho) )
    ...
    def step(
        ...
        # Compute neoclassical resistivity profile
        ...
        # D(rho) = eta_parallel / (mu_0 a^2)
        D = eta_prof / (MU_0 * self.a**2)

        # Build tridiagonal system for (I - dt/2 L) psi^{n+1} = (I + dt/2 L) psi^n + dt S
        # L(psi)_i = D_i * [ (psi_{i+1} - 2psi_i + psi_{i-1})/drho^2 + (psi_{i+1} - psi_{i-1})/(2 rho_i drho) ]
        ...
        # Axis (i=0) boundary condition: L(psi)_0 = 4 D_0 (psi_1 - psi_0) / drho^2
        ...
        for i in range(1, self.nr - 1):
            ...
            # RHS vector (I + dt/2 L) psi^n + dt S
        ...
        # Edge (i=nr-1) boundary condition: psi(edge) = constant (Dirichlet)
        # Wait, if psi(edge) is fixed, we just set diag=1, rhs=psi[edge]
        # Or better, we can maintain the boundary psi (which is typically 0)
        diag[-1] = 1.0
        sub[-1] = 0.0
        rhs[-1] = self.psi[-1]
