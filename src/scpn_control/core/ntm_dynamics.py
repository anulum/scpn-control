# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Modified Rutherford Equation & NTM Dynamics
# ──────────────────────────────────────────────────────────────────────
...
def eccd_stabilization_factor(d_cd: float, w: float) -> float:
    """
    ECCD stabilization efficiency factor.
...
def find_rational_surfaces(
    ...
    for m in range(1, m_max + 1):
        for n in range(1, n_max + 1):
            q_target = m / n
            # Find zero crossings of (q - q_target)
            q_diff = q - q_target
            crossings = np.where(np.diff(np.sign(q_diff)))[0]

            for idx in crossings:
                # Linear interpolate to find precise rho
                ...
                # Interpolate shear: s = (rho/q) * dq/drho
...
class NTMIslandDynamics:
    ...
    def __init__(
        ...
        self.R0 = R0
        self.B0 = B0
        # Typical classically stable tearing mode index
    ...
    def dw_dt(
        ...
        """
        Evaluate the RHS of the MRE.
        """
        if w <= 1e-6:
            # Island too small, stabilized by w_pol and polarization effects
            return 0.0
        ...
        # ECCD replaces bootstrap current, so it's stabilizing (negative)
...
class NTMController:
    ...
    def step(self, w: float, rho_rs: float, max_power: float = 20.0) -> float:
        """
        Check if intervention is needed. Returns requested ECCD power in MW.
        """
