# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Current Drive Physics Module
# ──────────────────────────────────────────────────────────────────────
...
class ECCDSource:
    ...
    def P_absorbed(self, rho: np.ndarray) -> np.ndarray:
        """Absorbed power density profile [W/m^3] (assumed normalized integral)."""
        if self.sigma_rho <= 0.0:
            return np.zeros_like(rho)
        P_W = self.P_ec_MW * 1e6
        # Simplified Gaussian power density
        P_dens = (
...
class NBISource:
    ...
    def j_cd(self, rho: np.ndarray, ne_19: np.ndarray, Te_keV: np.ndarray, Ti_keV: np.ndarray) -> np.ndarray:
        ...
        # Beam parameters (assume Deuterium beam)
        A_beam = 2.0
        Z_beam = 1.0
        m_beam = A_beam * M_P
        ...
        for i in range(len(rho)):
            ...
            # Fast ion density (steady state: n_fast = P_heat * tau_s / E_beam)
...
class CurrentDriveMix:
    ...
    def total_driven_current(self, rho: np.ndarray, ne: np.ndarray, Te: np.ndarray, Ti: np.ndarray) -> float:
        ...
        I_cd = integral( j_cd * 2 * pi * r * dr )
        """
        j_tot = self.total_j_cd(rho, ne, Te, Ti)
        # r = rho * a
        # dr = drho * a
        # dA = 2 * pi * r * dr = 2 * pi * rho * a^2 * drho
