...
# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Test Snn Pyo3 Bridge
...
try:
    import scpn_control_rs  # type: ignore[import-untyped]
...
class TestPySnnPool:
    def test_positive_error_positive_output(self) -> None:
        ...
        pool = scpn_control_rs.PySnnPool(n_neurons=50, gain=10.0, window_size=20)
        output = 0.0
        for _ in range(50):
    ...
    def test_zero_neurons_raises(self) -> None:
        ...
        with pytest.raises(ValueError):
            scpn_control_rs.PySnnPool(n_neurons=0, gain=1.0, window_size=10)

    def test_nan_gain_raises(self) -> None:
        """NaN gain must raise ValueError."""
        with pytest.raises(ValueError):
            scpn_control_rs.PySnnPool(n_neurons=10, gain=float("nan"), window_size=10)

    def test_zero_window_raises(self) -> None:
        """window_size=0 must raise ValueError."""
        with pytest.raises(ValueError):
            scpn_control_rs.PySnnPool(n_neurons=10, gain=1.0, window_size=0)

    def test_nan_error_raises(self) -> None:
        """NaN error input must raise ValueError."""
        pool = scpn_control_rs.PySnnPool(n_neurons=10, gain=1.0, window_size=10)
        with pytest.raises(ValueError):
            pool.step(float("nan"))

    def test_output_is_finite(self) -> None:
        """Output must be a finite float for valid inputs."""
        pool = scpn_control_rs.PySnnPool(n_neurons=20, gain=5.0, window_size=10)
        for _ in range(30):
            out = pool.step(1.0)
...
class TestPySnnController:
    def test_returns_control_signals(self) -> None:
        ...
        ctrl = scpn_control_rs.PySnnController(target_r=6.2, target_z=0.0)
    ...
    def test_getters(self) -> None:
        """Property getters should reflect constructor arguments."""
        ctrl = scpn_control_rs.PySnnController(target_r=6.5, target_z=-0.3)
    ...
    def test_nan_target_raises(self) -> None:
        ...
        with pytest.raises(ValueError):
            scpn_control_rs.PySnnController(target_r=float("nan"), target_z=0.0)
...
class TestRustCompatWrappers:
    ...
    def test_rust_snn_pool_wrapper(self) -> None:
        from scpn_control.core._rust_compat import RustSnnPool
