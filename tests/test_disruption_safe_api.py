...
# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Test Disruption Safe Api
...
class TestPredictDisruptionRiskSafeWithTorch:
    ...
    def test_deterministic(self):
        ...
        r1, _ = predict_disruption_risk_safe(signal, allow_legacy_fallback=True)
        r2, _ = predict_disruption_risk_safe(signal, allow_legacy_fallback=True)
        assert r1 == r2

    def test_meta_contains_mode(self):
        signal = np.ones(100)
        _, meta = predict_disruption_risk_safe(signal, allow_legacy_fallback=True)
        assert "mode" in meta
