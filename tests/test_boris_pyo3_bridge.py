...
# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Test Boris Pyo3 Bridge
...
class TestBorisAdvance:
    def test_preserves_speed_pure_b(self) -> None:
        ...
        advanced = scpn_control_rs.py_advance_boris(particles, e_field, b_field, dt, steps)
...
class TestHeatingProfile:
    def test_positive(self) -> None:
        ...
        e_field = [0.0, 0.0, 0.0]
        b_field = [0.0, 0.0, 5.0]
        advanced = scpn_control_rs.py_advance_boris(particles, e_field, b_field, 1e-9, 50)

        profile = scpn_control_rs.py_get_heating_profile(
            advanced,
            nr=33,
            nz=33,
            r_min=2.0,
...
class TestPopulationSummary:
    def test_fields(self) -> None:
        ...
        particles = _seed(32)
        summary = scpn_control_rs.py_particle_population_summary(particles, 0.1)
...
class TestGyroradius:
    def test_order_of_magnitude(self) -> None:
        ...
        advanced = scpn_control_rs.py_advance_boris(particles, e_field, b_field, 1e-10, 1000)
