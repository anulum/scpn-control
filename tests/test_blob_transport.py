...
def test_sol_blob_profile():
    # Without blobs
    prof_clean = SOLBlobProfile.radial_density(r=0.05, Gamma_blob=0.0, D_perp=1.0, lambda_n=0.02)
    
    # With blobs
    prof_blobs = SOLBlobProfile.radial_density(r=0.05, Gamma_blob=1e20, D_perp=1.0, lambda_n=0.02)
    
    # Profile should be broader (higher density far out)
    assert prof_blobs > prof_clean
