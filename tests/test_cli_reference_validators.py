...
# SCPN Control — CLI reference-validator module contract tests

"""Module-surface tests for :mod:`scpn_control.cli_reference_validators`.

The per-command behaviour is exercised through the root ``scpn-control`` group in
the ``test_cli_*`` suites; this file guards the module's own contract: the CSV
option helper and the ``REFERENCE_VALIDATOR_COMMANDS`` registration tuple that
`scpn_control.cli` folds onto the root group.
"""
...
from scpn_control.cli_reference_validators import (
    REFERENCE_VALIDATOR_COMMANDS,
    _split_csv_option,
)
