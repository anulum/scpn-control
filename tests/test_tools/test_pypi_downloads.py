...
# SCPN Control — bounded PyPI metrics tests
"""Tests for SCPN Control's fail-closed PyPI download history."""
...
_SPEC = importlib.util.spec_from_file_location("scpn_control_pypi_downloads", _MODULE_PATH)
...
def test_detect_package_reads_project_contract(tmp_path: Path) -> None:
    ...
    assert downloads.detect_package(pyproject) == "scpn-control"
...
def test_project_writer_rejects_every_alternate_target_before_fetch(
    ...
    downloads.require_project_csv_target("scpn-control", Path("downloads/scpn-control.csv"))
    for package, csv_path in (
        ("other", Path("downloads/scpn-control.csv")),
        ("scpn-control", Path("downloads/other.csv")),
        ("scpn-control", Path("downloads/../downloads/scpn-control.csv")),
        ("scpn-control", tmp_path / "downloads/scpn-control.csv"),
    ):
...
def test_http_get_uses_fixed_host_bounded_read_and_closes(
    ...
    assert downloads._http_get("scpn-control") == b'{"ok": true}'
    assert connection.request_args == (
        "GET",
        "/api/packages/scpn-control/overall",
    )
...
def test_http_get_fails_closed_and_closes(
    ...
    with pytest.raises(error_type, match=message):
        downloads._http_get("scpn-control")
    assert connection.closed
...
def test_fetch_overall_rejects_invalid_json_contract(payload: bytes) -> None:
    ...
    with pytest.raises(downloads.DownloadSnapshotError):
        downloads.fetch_overall("scpn-control", lambda package: payload)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            b'"downloads":19}],"package":"other","package":"scpn-control",'
            b'"type":"overall_downloads"}'
        ),
        (
            b'{"data":[{"category":"with_mirrors","date":"2026-07-17",'
            b'"downloads":1,"downloads":19}],"package":"scpn-control",'
            b'"type":"overall_downloads"}'
        ),
    ],
)
def test_duplicate_remote_object_names_fail_before_csv_write(
    ...
    csv_path = tmp_path / "downloads" / "scpn-control.csv"
    assert (
        downloads.main(
            ["--package", "scpn-control", "--csv", str(csv_path)],
...
def test_remote_schema_fails_closed_on_identity_or_partial_data(
    ...
    with pytest.raises(downloads.DownloadSnapshotError, match=message):
        downloads.fetch_overall("scpn-control", lambda package: _payload_bytes(payload))
...
def test_fetch_overall_returns_complete_validated_rows() -> None:
    ...
    assert downloads.fetch_overall("scpn-control", lambda package: _payload_bytes(_SAMPLE)) == {
...
def test_remote_schema_preserves_legitimate_sparse_without_mirrors() -> None:
    ...
    assert downloads.fetch_overall("scpn-control", lambda package: _payload_bytes(payload))["2026-07-17"] == {
        "with_mirrors": 19
    }
...
def test_missing_csv_and_empty_summary_are_explicit(tmp_path: Path) -> None:
    ...
    assert downloads.summary("scpn-control", {}) == ("scpn-control: no download data available yet")
    assert "without_mirrors=n/a" in downloads.summary("scpn-control", {"2026-07-17": {"with_mirrors": 19}})
...
def test_main_failure_keeps_existing_series(
    ...
    assert downloads.main(["--package", "scpn-control", "--csv", str(csv_path)], fail_fetch) == 1
...
def test_main_print_package_and_requires_csv(
    ...
    assert capsys.readouterr().out.strip() == "scpn-control"
    with pytest.raises(SystemExit) as exc_info:
        downloads.main(["--package", "scpn-control"])
...
def test_transient_failures_retry_then_soft_skip(
    ...
    result = downloads.fetch_overall_with_retry("scpn-control", recover, waits.append)
...
def test_workflow_has_one_push_only_bounded_writer_and_pinned_actions() -> None:
    ...
    assert 'readonly package="scpn-control"' in workflow
    assert 'readonly csv_path="downloads/scpn-control.csv"' in workflow
    ...
    assert "scpn-control-metrics/downloads/scpn-control.csv" in workflow
