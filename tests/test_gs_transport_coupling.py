...
# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Test Gs Transport Coupling
...
# ──────────────────────────────────────────────────────────────────────
# SCPN Control — GS ↔ Transport Self-Consistency Tests
# © 1998–2026 Miroslav Šotek. All rights reserved.
...
def test_self_consistent_psi_residuals_recorded(solver: TransportSolver):
    ...
    assert len(result["psi_residuals"]) == n_done, f"Expected {n_done} residuals, got {len(result['psi_residuals'])}"
...
def test_solve_equilibrium_called_during_self_consistent(
    ...
    with patch.object(solver, "solve_equilibrium", wraps=solver.solve_equilibrium) as mock_eq:
        ...
        assert mock_eq.call_count == 2, f"Expected 2 calls to solve_equilibrium, got {mock_eq.call_count}"
