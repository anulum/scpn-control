...
# SCPN Control — Tests: Native TGLF-Equivalent Transport Model
...
def _make_linear(
    ...
    modes = [
        EigenMode(k_y_rho_s=k, omega_r=w, gamma=g, mode_type=mt) for k, g, w, mt in zip(k_y, gamma, omega_r, mode_type)
    ]
...
class TestTrappedParticle:
    ...
    def test_damping_less_than_one(self):
        p = GKLocalParams(R_L_Ti=6.9, R_L_Te=6.9, R_L_ne=2.2, q=1.4, s_hat=0.78, nu_star=0.5, epsilon=0.18)
