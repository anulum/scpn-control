...
# SCPN Control — Test Torax Hybrid Loop.
...
class TestToraxPlasmaState:
    """Exercise TORAX plasma-state immutability."""

    def test_frozen(self):
        """Reject mutation of an immutable plasma-state value object."""
...
class TestToraxStep:
    """Exercise one bounded TORAX plasma-state transition."""

    def test_returns_state(self):
        """Return a new plasma state after one transition."""
    ...
    def test_clipped_ranges(self):
        """Keep the TORAX step result inside declared physical ranges."""
        rng = np.random.default_rng(0)
        s = ToraxPlasmaState(beta_n=1.85, q95=4.9, li3=0.95, w_thermal_mj=140.0)
        for _ in range(200):
...
class TestBuildHybridController:
    """Exercise the hybrid controller production builder."""
    ...
    def test_builds_successfully(self):
        """Construct the hybrid controller from its production builder."""
...
class TestRunCampaign:
    """Exercise deterministic hybrid-loop campaign execution."""

    def test_deterministic(self):
        """Produce identical campaign results for identical inputs."""
    ...
    def test_result_fields(self):
        """Expose every declared campaign result field."""
    ...
    def test_bad_episodes_raises(self):
        """Reject a campaign with an invalid episode count."""
        with pytest.raises(ValueError, match="episodes"):
            run_nstxu_torax_hybrid_campaign(episodes=0)

    def test_bad_steps_raises(self):
        """Reject a campaign with an invalid step count."""
    ...
    def test_longer_campaign(self):
        """Run a multi-episode campaign through the full loop."""
