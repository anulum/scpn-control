...
class TestClosedLoop:
    """Integration tests: full observer-feedback loop.

    Plant discretised with exact ZOH (matrix exponential) to avoid
    forward-Euler instability on the stiff vertical stability model
    (eigenvalue ~+100).
    """
    ...
    def test_large_dt_stable(self, ctrl):
        """dt=0.07 (40% above flight-sim default): must not diverge.

        Open-loop plant amplifies ~780x per step at this dt.
        """
        dt = 0.07
        A, B2, C2 = ctrl.A, ctrl.B2, ctrl.C2
        Ad, Bd = _zoh_discretize(A, B2, dt)
    ...
    def test_small_dt_stable(self, ctrl):
        """dt=0.001 at 1 kHz: must converge quickly."""
        dt = 0.001
        A, B2, C2 = ctrl.A, ctrl.B2, ctrl.C2
        Ad, Bd = _zoh_discretize(A, B2, dt)
    ...
    def test_disturbance_rejection(self, ctrl):
        """10 MA kick at t=5s must be rejected."""
        dt = 0.05
    ...
    def test_independent_instances(self):
        """Two controllers maintain independent observer states."""
