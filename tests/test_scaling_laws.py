...
class TestIPB98y2:
    ...
    def test_rejects_zero_current(self, coefficients):
        with pytest.raises(ValueError):
            ipb98y2_tau_e(
                Ip=0.0, BT=5.0, ne19=5.0, Ploss=50.0, R=6.0, kappa=1.5, epsilon=0.3, coefficients=coefficients
            )

    def test_rejects_negative_field(self, coefficients):
        with pytest.raises(ValueError):
            ipb98y2_tau_e(
                Ip=15.0, BT=-5.0, ne19=5.0, Ploss=50.0, R=6.0, kappa=1.5, epsilon=0.3, coefficients=coefficients
            )

    def test_rejects_nan(self, coefficients):
        with pytest.raises(ValueError):
            ipb98y2_tau_e(
                Ip=float("nan"), BT=5.0, ne19=5.0, Ploss=50.0, R=6.0, kappa=1.5, epsilon=0.3, coefficients=coefficients
            )

    def test_rejects_inf(self, coefficients):
        with pytest.raises(ValueError):
            ipb98y2_tau_e(
                Ip=float("inf"), BT=5.0, ne19=5.0, Ploss=50.0, R=6.0, kappa=1.5, epsilon=0.3, coefficients=coefficients
            )
