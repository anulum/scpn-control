...
# ──────────────────────────────────────────────────────────────────────
# SCPN Control — Test Jax Neural Equilibrium
...
# ──────────────────────────────────────────────────────────────────────
# Tests for JAX-accelerated neural equilibrium
# ──────────────────────────────────────────────────────────────────────
...
from scpn_control.core.neural_equilibrium import MinimalPCA, SimpleMLP

jax = pytest.importorskip("jax")
jnp = pytest.importorskip("jax.numpy")

from scpn_control.core.jax_neural_equilibrium import (
    _mlp_forward_jax,
...
class TestMLPForwardParity:
    def test_single_sample(self, rng: np.random.Generator, mlp: SimpleMLP, jax_params: tuple) -> None:
...
class TestFullPipelineParity:
    def test_single_feature(
        ...
        # JAX pipeline
        psi_jax = jax_neural_eq_predict(features, jax_params[0], jax_params[1], jax_params[2], grid_shape=(65, 65))
...
class TestBatchedVmap:
    def test_vmap_matches_loop(self, rng: np.random.Generator, jax_params: tuple) -> None:
        ...
        results = []
        for i in range(6):
            psi = jax_neural_eq_predict(features[i], jax_params[0], jax_params[1], jax_params[2], grid_shape=(65, 65))
            results.append(psi)
    ...
    def test_output_shape(self, rng: np.random.Generator, jax_params: tuple) -> None:
        ...
        psi = jax_neural_eq_predict_batched(features, jax_params[0], jax_params[1], jax_params[2], grid_shape=(65, 65))
