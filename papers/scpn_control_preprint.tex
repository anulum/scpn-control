...
On the CI reference runner (AMD EPYC 7763) the NumPy SNN controller steps in
\SI{23.2}{\micro\second} median (P50); with the Rust backend this drops to
\SI{0.92}{\micro\second} (P50), and the full native integrated control cycle runs
in \SI{5.05}{\micro\second} (P50) --- competitive with the DIII-D PCS physics
loop rate of 4--10~kHz. All figures are reproducible CI measurements
...
\begin{enumerate}
  ...
  \item A Rust-accelerated runtime achieving a \SI{5.05}{\micro\second}
        P50 native integrated control cycle (and a \SI{0.92}{\micro\second}
        isolated Rust SNN step) on the CI reference runner, validated against
        DIII-D and SPARC equilibria at $<2\%$ RMSE
        (Section~\ref{sec:validation}).
...
such rather than estimated. The local workstation (Intel i5-11600K) is faster by
roughly $1.5\times$ and is published side by side in the repository.

\begin{table}[tb]
\centering
\caption{Control-loop latency, CI reference runner (AMD EPYC 7763), 5000 steps;
isolated per-controller step except the final integrated row.}
\label{tab:timing}
\begin{tabular}{llrrr}
\toprule
Controller & Backend & P50 (\si{\micro\second}) & P99 (\si{\micro\second})
  & Throughput \\
\midrule
PID          & NumPy & 0.43  & 0.49  & 2320~kHz \\
SNN          & NumPy & 23.2  & 40.5  & 43~kHz  \\
SNN          & Rust  & 0.92  & 1.06  & 1085~kHz \\
MPC ($N_p{=}10$) & internal QP & 23741 & 25160 & 0.04~kHz \\
MPC ($N_p{=}10$) & acados & \multicolumn{3}{l}{not installed (see text)} \\
$H_\infty$   & NumPy (general SS) & 28.5 & 48.6 & 35~kHz \\
$H_\infty$   & Rust (2-state VDE) & 1.32 & 1.73 & 756~kHz \\
\midrule
Integrated control cycle & Rust native & 5.05 & 6.34 & 198~kHz \\
\bottomrule
\end{tabular}
\end{table}

The Rust SNN backend (1085~kHz) and the native integrated control cycle
(198~kHz) exceed the DIII-D PCS physics loop rate of 4--10~kHz by two orders of
magnitude. The $H_\infty$ controller meets real-time bands on either backend.
The nonlinear MPC reaches real-time only with a compiled QP backend: its
microsecond regime requires \texttt{acados}, which is not installed in the CI or
local environments, so only the pure-Python QP backends (millisecond-scale) are
reported here. That row is therefore not a real-time claim until the
\texttt{acados} backend is measured.
...
(v) a plasma-native 8-layer $K_{nm}$ grounded in
