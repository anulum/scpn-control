...
\SI{5.05}{\micro\second} P50 on the CI reference runner, reproducible;
...
\begin{enumerate}
  ...
  \item A CPU-only Rust-accelerated runtime whose control compute sits well
        within the real-time budget (\SI{5.05}{\micro\second} P50 native
        integrated control cycle on the CI reference runner), with equilibrium
        models validated against DIII-D and SPARC at $<2\%$ RMSE
