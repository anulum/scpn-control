...
Use this report to verify which physics claims are admissible in each release train.

- Confirm traceability status before promoting any claim to marketing, pitch, or investor-facing text.
- Use unresolved entries here as the basis for next validation tasks.
- Treat generated JSON-driven statuses as the source of truth for evidence boundaries.
