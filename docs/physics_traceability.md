...
- Status: pass
- Registry entries: 72
- Open fidelity gaps: 71
- Full-fidelity public claims blocked: 71
- Resolved module paths: 72
- Resolved evidence paths: 649
- External validation trackers: 8
- Source marker coverage: 42/42
...
- Plasma-control and facility replay artefacts: [#51](https://github.com/anulum/scpn-control/issues/51) — 17 open claim(s) (validation_gap=1, bounded_model=16) — RZIP, RWM, free-boundary, density, digital twin, SOC learning, burn, volt-second, static structured-mu analysis, future D-K synthesis, and reduced-plant replay evidence.
...
- Hardware, HDL, CODAC/EPICS, and runtime deployment evidence: [#53](https://github.com/anulum/scpn-control/issues/53) — 7 open claim(s) (bounded_model=7) — Vivado, Quartus, Yosys, timing closure, simulator evidence, CODAC/EPICS timing, interlocks, backpressure, HIL replay, and runtime parity.
...
| `src/scpn_control/core/imas_adapter.py` | Equilibrium interchange claims must preserve the solver COCOS-1 convention, SI units, strictly ordered R/Z grids, (Z,R) solver field orientation, IMAS (R,Z) IDS orientation, DD3 COCOS-11 j_tor and DD4 COCOS-17 j_phi current-leaf semantics, indexed time and vacuum-field arrays, reference radius, plasma current, and source provenance without fabricated machine defaults or current density. | IMAS Data Dictionary equilibrium IDS and COCOS conventions; IMAS-Python IDSFactory and DBEntry contracts; OMAS ODS COCOS input/output conversion contract | R and Z in metres; poloidal flux in webers per radian in solver COCOS 1; toroidal current density in A/m^2; plasma current in amperes; vacuum toroidal field in tesla; time in seconds; two-dimensional solver fields ordered (Z,R) and IMAS/OMAS rectangular fields ordered (R,Z). | tests/test_imas_adapter.py validates the immutable neutral contract, strict kernel metadata, GEQDSK absence semantics, and compatibility boundary; tests/test_imas_adapter_imas.py executes real DD3/DD4 IDSFactory objects and a real NetCDF DBEntry round trip; tests/test_imas_adapter_omas.py executes real ODS round trips, COCOS conversion, multiple times, and fail-closed metadata paths; tests/test_imas_ods_contract.py exercises representative real ODS profile and time-array structures; Focused adapter coverage executes 363 statements and 90 branches at 100 percent | reference_validated |  |
...
| `src/scpn_control/control/static_mu_analysis.py` | Static mu-analysis claims must declare the structured uncertainty blocks, positive block bounds, separately designed Riccati state-feedback gain, bound-scaled closed-loop DC map at zero frequency, D-scaling upper-bound fit, finite controller state/timestep domains, and the exclusion of frequency-dependent H-infinity D-K synthesis. | Doyle 1982 structured singular value definition; Packard and Doyle 1993 the complex structured singular value; Balas et al. 1993 mu-analysis and synthesis toolbox; Skogestad and Postlethwaite 2005 multivariable feedback control; Repository fail-closed static mu-analysis claim-admission contract | State, control, output, and uncertainty units inherit the supplied state-space plant contract; mu, uncertainty bounds, and D-scalings are dimensionless. | tests/test_static_mu_analysis.py D-scaled upper-bound behaviour, uncertainty-bound scaling, finite-domain rejection, closed-loop plant dependence, claim-admission, persisted digest, tamper, and duplicate-key checks; tests/test_static_mu_analysis_contract.py canonical naming, algorithm-parameter, independent DC-map, compatibility, and 100 percent owner coverage checks; validation/validate_mu_structured_singular_value.py closed-form exact-identity validation of compute_static_mu_upper_bound (full-block mu=sigma_max, diagonal mu=max\|M_ii\|, rank-one mu=sum\|u_i v_i\|, and the rho<=mu<=sigma_max sandwich) with tamper-evident sealed evidence; tests/test_mu_structured_singular_value_validation.py exact-identity, sandwich, determinism, diagnostic-not-gated, and evidence-seal checks; validation/validate_static_mu_analysis_reference.py strict static mu-analysis reference artefact gate; validation/benchmark_static_mu_analysis_claims.py bounded claim-admission benchmark; docs/learning/control_theory_primer.md states the bounded static analysis domain | bounded_model | [#51](https://github.com/anulum/scpn-control/issues/51) |
...
| `src/scpn_control/control/density_controller.py` | Density-control claims must declare the Greenwald limit, ITER operating margin, radial particle transport grid, gas-puff source, pellet source, NBI source, cryopump sink, recycling source, controller gains, CFL-limited explicit transport step, and the bounded circular interferometer projection C_ij = 2 a [sqrt(max(rho_(j+1)^2-b_i^2,0)) - sqrt(max(rho_j^2-b_i^2,0))]. Geometry/transport/actuator/diagnostic provenance, source integral, particle-inventory change, and matched Greenwald-fraction and inventory reference tolerances remain mandatory before facility-calibrated density-control claims. | Greenwald 2002 density limit; ITER Physics Basis 1999 density operating margin; Parks and Turnbull 1978 neutral gas shielding pellet-ablation model; Milora 1995 neutral gas shielding pellet-ablation model; Milanese et al. 2020 multi-chord interferometry and Abel reconstruction, Physics of Plasmas 27 042516, doi:10.1063/1.5142638 | SI particles per second, metres, seconds, m^-3 density, metre-valued annular chord weights, m^-2 line-integrated density, m^2/s, m/s, pellet millimetres, beam keV, megawatts, and dimensionless Greenwald fraction and signed normalised chord impact. | tests/test_density_controller.py Greenwald, NGS pellet trajectory deposition, sink, controller, geometry, transport-profile, source-input, timestep guard, exact annular chord projection, independent Kalman/Joseph reference, covariance stability, and fail-closed density-control claim evidence checks; tests/test_pellet_injection.py Parks-Turnbull NGS ablation and pellet trajectory checks; validation/benchmark_density_control_claims.py publishes deterministic bounded synthetic claim evidence with explicit facility-calibrated claim exclusion; validation/reports/density_control_claims.json records bounded geometry, transport, actuator, diagnostic, CFL, Greenwald, source-integral, and inventory evidence; validation/validate_density_reference.py strict density reference artefact gate; validation/validate_density_control.py validates the Greenwald limit I_p/(pi a^2), the volume-averaged Greenwald fraction, the circular flux-surface volume elements V' and V, the gas-puff, neutral-beam, and recycling source normalisation (particle conservation of the source integral), the cryopump edge sink, the diffusion operator vanishing on a uniform interior, the Greenwald scaling laws, and the exact uniform-density circular chord projection and signed-impact symmetry, with tamper-evident sealed evidence and runtime source SHA-256; tests/test_density_control_validation.py Greenwald-limit, Greenwald-fraction, volume-element, source-conservation, cryopump-sink, diffusion-invariance, interferometer-projection, scaling-law, configuration-guard, runtime-source-digest, and evidence-seal checks | bounded_model | [#51](https://github.com/anulum/scpn-control/issues/51) |
...
| `src/scpn_control/phase` | Layer-ontology claims must keep Paper 27 abstract indices separate from plasma-labelled positions, restrict the built-in plasma hierarchy to reduced ordered prefixes L=1..8 or the explicit refined L=16 mapping, preserve matrix-label-frequency dimensional agreement, and describe adaptive coupling as a bounded model heuristic rather than an identified reactor law. | Repository Paper 27 abstract Knm construction; Repository illustrative plasma-labelled hierarchy contract; Repository bounded adaptive Knm model contract | Layer indices and labels are distinct categorical positions; illustrative natural frequencies are in radians per second, coupling and phase-amplitude gains are dimensionless by the declared model convention, and all matrix, label, frequency, and diagnostic-vector dimensions must agree. | tests/test_plasma_knm.py supported hierarchy, dimension, label, frequency, mode, and invalid-domain checks; tests/test_adaptive_knm.py bounded update, rate-limit, guard, and reset checks; tests/test_adaptive_knm_diagnostics.py diagnostic-input usage checks; tests/test_paper27_reference_boundaries.py public ontology-separation checks | bounded_model | [#53](https://github.com/anulum/scpn-control/issues/53) |
...
| `src/scpn_control/control/state_estimator.py` | Nonlinear state-estimation claims must declare the process and measurement functions, analytic Jacobians, state and measurement ordering, control-input convention, process-noise discretisation, measurement residual, state-addition or retraction operation, covariance assumptions, timestep domain, and innovation-conditioning failure boundary. | Simon 2006 Optimal State Estimation Chapter 13 extended Kalman filter; Lister et al. 1997 real-time magnetic equilibrium reconstruction; Moreau et al. 2008 observer-based tokamak profile estimation; Repository nonlinear estimator numerical contract | The compatible default state is R and Z in metres, vR and vZ in metres per second, plasma current Ip in mega-amperes, and core electron temperature in kiloelectronvolts; custom model units, control ordering, residual conventions, and covariance units are caller-declared and must remain internally consistent. | tests/test_state_estimator.py independently calculated nonlinear prediction, analytic-Jacobian covariance propagation, Cholesky gain, Joseph correction, wrapped residual and state retraction, fail-closed callback contracts, and 100 percent changed-owner statement and branch coverage; tests/test_free_boundary_tracking.py and tests/test_free_boundary_tracking_variants.py compatible production consumer checks for the default six-state/four-diagnostic model | bounded_model | [#51](https://github.com/anulum/scpn-control/issues/51) |
| `src/scpn_control/core/gk_nonlinear.py` | Five-dimensional delta-f flux-tube Vlasov evolution with E cross B bracket, ballooning connection, kinetic electrons, and Sugama collision terms. | Dimits et al. 2000 Cyclone Base Case; Sugama et al. 2006 collision operator | Gyro-Bohm-normalised heat flux and chi_i with explicit R/L_Ti, rho_s, c_s, and reference gradient normalisation. | ROADMAP.md v0.18.0 open revalidation item; papers/submissions/001_neuro_symbolic_tokamak_control_software/manuscript.md nonlinear GK validation limitation; validation/gk_nonlinear_cyclone.py saturation-evidence assessor; validation/reference_data/cyclone_base/cyclone_nonlinear_chi_i_reference.json obtained published cross-code reference (Dimits et al. 2000, verified at source): offset-linear fit chi_i L_n/(rho_i^2 v_ti) = 15.4(1 - 6.0 L_T/R) giving ~2.0 at R/L_T=6.9, Dimits shift (linear critical R/L_T ~4.0, nonlinear effective ~6.0), and cross-model factors, with an explicit unit-conversion caveat to the module's chi_i_gB normalisation | validation_gap | [#47](https://github.com/anulum/scpn-control/issues/47) |
| `src/scpn_control/control/h_infinity_controller.py` | H-infinity claims must declare the normalized continuous-time standard plant with D11=D22=0, explicit D12/D21, stabilizability and detectability, D12^T C1=0, D12^T D12=I, B1 D21^T=0, D21 D21^T=I, both positive-semidefinite stabilizing Riccati solutions, strict rho(XY)<gamma^2, the complete central dynamic controller realization, and the unsaturated continuous-time claim boundary. Exact controller ZOH, output clipping, and Rust runtime execution must not be represented as arbitrary sampled-data, anti-windup, independent Rust-synthesis, facility, structured-uncertainty, or D-K guarantees. | Doyle, Glover, Khargonekar and Francis 1989 state-space H-infinity solution, IEEE TAC 34(8), Theorem 3, doi:10.1109/9.29425; Normalized bounded-real and small-gain contracts | State, disturbance, control, measurement, and performance units inherit the declared plant; gamma and normalization residuals are dimensionless; continuous-time matrices use seconds; runtime dt uses seconds. | tests/test_h_infinity_controller.py normalized assumptions, central-formula identity, Riccati and spectral admission, fail-closed domains, exact ZOH, MIMO, and claim-boundary checks; tests/test_h_infinity_closed_loop.py independent augmented realization, dense frequency-response corroboration, sampled example convergence, and disturbance rejection; tests/test_h_infinity_validation.py sealed evidence and tamper checks; tests/test_rust_compat_wrapper.py exact Python/Rust same-realization parity; validation/validate_h_infinity_control.py bounded DGKF validator; validation/reports/h_infinity_control.json sealed bounded-model evidence | bounded_model | [#51](https://github.com/anulum/scpn-control/issues/51) |
...
### backend-neutral IMAS-Python and OMAS equilibrium interchange

- Fidelity status: `reference_validated`
- Module path: `src/scpn_control/core/imas_adapter.py`
- Full-fidelity public claim: allowed
- External validation tracker: none
- Covered source paths: 1
- Claim admission requirements:
  - Keep real IMAS-Python DD3/DD4 IDS validation and NetCDF DBEntry round trips executing in the authoritative CI lane
  - Keep real OMAS DD3 ODS round trips and solver-convention checks executing in the authoritative CI lane
  - Restrict public claims to software interchange; facility connectivity, reconstruction accuracy, and control validation require separate evidence
...
- Fidelity status: `bounded_model`
- Module path: `src/scpn_control/control/static_mu_analysis.py`
- Full-fidelity public claim: blocked
- External validation tracker: [#51](https://github.com/anulum/scpn-control/issues/51) — Plasma-control and facility replay artefacts
- Covered source paths: 1
- Claim admission requirements:
...
### illustrative phase-layer ontology and adaptive coupling contract

- Fidelity status: `bounded_model`
- Module path: `src/scpn_control/phase`
- Full-fidelity public claim: blocked
- External validation tracker: [#53](https://github.com/anulum/scpn-control/issues/53) — Hardware, HDL, CODAC/EPICS, and runtime deployment evidence
- Covered source paths: 2
- Claim admission requirements:
  - Keep Paper 27 abstract layers separate from the hand-selected plasma-labelled hierarchy and reject implicit plasma ontologies outside reduced L=1..8 and refined L=16
  - Require a released phase-orchestration domain binding, identified reactor observation-to-oscillator mapping, coupled FUSION plant, physical actuator contract, and matched closed-loop baselines before any reactor phase-control claim
  - Identify and validate scenario-specific adaptive coupling parameters before treating model-local updates as physical reactor dynamics
...
### nonlinear extended Kalman state estimator

- Fidelity status: `bounded_model`
- Module path: `src/scpn_control/control/state_estimator.py`
- Full-fidelity public claim: blocked
- External validation tracker: [#51](https://github.com/anulum/scpn-control/issues/51) — Plasma-control and facility replay artefacts
- Covered source paths: 1
- Claim admission requirements:
  - Bind every promoted result to an exact nonlinear plant and diagnostic model, analytic Jacobian implementation, state and measurement convention, covariance model, sampled input, tolerance, failure count, and source digest
  - Validate estimation error and uncertainty calibration against authorised measured diagnostic replay or an independently maintained reference estimator before facility-accuracy claims
...
- Fidelity status: `bounded_model`
- Module path: `src/scpn_control/control/h_infinity_controller.py`
- Full-fidelity public claim: blocked
- External validation tracker: [#51](https://github.com/anulum/scpn-control/issues/51) — Plasma-control and facility replay artefacts
- Covered source paths: 1
- Claim admission requirements:
  - Keep every public claim restricted to the normalized unsaturated continuous-time standard plant and its strict DGKF existence conditions
  - Require a matched SPO scenario contract and released FUSION plant, diagnostic, actuator, sampled-loop, saturation, and facility evidence before reactor-control or deployment claims
