...
- Status: pass
- Registry entries: 70
- Open fidelity gaps: 69
- Full-fidelity public claims blocked: 69
- Resolved module paths: 70
- Resolved evidence paths: 628
...
| `src/scpn_control/core/gs_phase_sync.py` | Reduced-order Paper-27 / ζ·sin(Ψ−θ) oscillator and multi-step Lyapunov helpers extracted from FusionKernel must declare the approximation, Kuramoto assumptions, and absence of reactor-signal identification, plant coupling, actuator mapping, and reactor-control authority. | Paper-27 phase reduction / ζ·sin(Ψ−θ) references; Kuramoto-Sakaguchi mean-field synchronisation literature; Repository FusionKernel dual-home phase-sync contract | Dimensionless oscillator phases, coupling strengths, and Lyapunov metrics in SI time for multi-step tracking helpers. | tests/test_gs_phase_sync.py; tests/test_fusion_kernel.py; tests/test_kuramoto_synchronisation_validation.py | bounded_model | [#51](https://github.com/anulum/scpn-control/issues/51) |
...
- Claim admission requirements:
  ...
  - Preserve mean-field Kuramoto benchmark evidence as oscillator-model evidence
  - Require reactor-specific signal identification, a coupled plant, an actuator map, and closed-loop validation before any plasma-phase control claim
...
- Covered source paths: 1
- Claim admission requirements:
  - Synchronisation onset and order-parameter metrics are validated against the exact mean-field Lorentzian Kuramoto results in validation/validate_kuramoto_synchronisation.py; this is oscillator-model evidence only
  - Require a named reactor scenario, identified observation-to-oscillator mapping, coupled plant, physical actuator contract, and matched closed-loop baselines before any plasma-phase control claim
